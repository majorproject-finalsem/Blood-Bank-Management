						...
						<tr>
							...
							<td><center><b>
								...
								if($a_pos > 0)
								{
									...
									$get_sum->execute();
									$data1=$get_sum->fetch(PDO::FETCH_ASSOC);
									$diff_qnt=$db->prepare("SELECT SUM(eqnt) AS diff FROM exchng_registration WHERE exchangebg='A+' || exchangebg='a+'");
									$diff_qnt->execute();
									$data2=$diff_qnt->fetch(PDO::FETCH_ASSOC);
									$addn_qnt=$db->prepare("SELECT SUM(dqnt) AS addn FROM exchng_registration WHERE donatedbg='A+' || donatedbg='a+'");
									$addn_qnt->execute();
									$data3=$addn_qnt->fetch(PDO::FETCH_ASSOC);
									echo $data1['total']+$data3['addn']-$data2['diff'];
								}
								else
								{
									echo $b_pos;
						...
						<tr>
							...
							<td><center><b>
								...
								if($b_pos > 0)
								{
									...
									$get_sum->execute();
									$data1=$get_sum->fetch(PDO::FETCH_ASSOC);
									$diff_qnt=$db->prepare("SELECT SUM(eqnt) AS diff FROM exchng_registration WHERE exchangebg='B+' || exchangebg='b+'");
									$diff_qnt->execute();
									$data2=$diff_qnt->fetch(PDO::FETCH_ASSOC);
									$addn_qnt=$db->prepare("SELECT SUM(dqnt) AS addn FROM exchng_registration WHERE donatedbg='B+' || donatedbg='b+'");
									$addn_qnt->execute();
									$data3=$addn_qnt->fetch(PDO::FETCH_ASSOC);
									echo $data1['total']+$data3['addn']-$data2['diff'];
								}
						...
						<tr>
							...
							<td><center><b>
								...
								if($ab_pos > 0)
								{
									...
									$get_sum->execute();
									$data1=$get_sum->fetch(PDO::FETCH_ASSOC);
									$diff_qnt=$db->prepare("SELECT SUM(eqnt) AS diff FROM exchng_registration WHERE exchangebg='AB+' || exchangebg='ab+'");
									$diff_qnt->execute();
									$data2=$diff_qnt->fetch(PDO::FETCH_ASSOC);
									$addn_qnt=$db->prepare("SELECT SUM(dqnt) AS addn FROM exchng_registration WHERE donatedbg='AB+' || donatedbg='ab+'");
									$addn_qnt->execute();
									$data3=$addn_qnt->fetch(PDO::FETCH_ASSOC);
									echo $data1['total']+$data3['addn']-$data2['diff'];
								}
						...
						<tr>
							...
							<td><center><b>
								...
								if($o_pos > 0)
								{
									...
									$get_sum->execute();
									$data1=$get_sum->fetch(PDO::FETCH_ASSOC);
									$diff_qnt=$db->prepare("SELECT SUM(eqnt) AS diff FROM exchng_registration WHERE exchangebg='O+' || exchangebg='o+'");
									$diff_qnt->execute();
									$data2=$diff_qnt->fetch(PDO::FETCH_ASSOC);
									$addn_qnt=$db->prepare("SELECT SUM(dqnt) AS addn FROM exchng_registration WHERE donatedbg='O+' || donatedbg='o+'");
									$addn_qnt->execute();
									$data3=$addn_qnt->fetch(PDO::FETCH_ASSOC);
									echo $data1['total']+$data3['addn']-$data2['diff'];
								}
